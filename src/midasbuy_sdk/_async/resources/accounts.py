...
class AsyncAccounts(AsyncResource):
    ...
    async def list(self, *, limit: int | None = None, offset: int = 0) -> Page[Account]:
        lim = self._limit(limit)
        data = await self._t.request(
            "GET", "/accounts/list", params={"limit": lim, "offset": offset}
        )
        return self._page(data, Account.model_validate, limit=lim, offset=offset)

    async def get(self, account_id: str) -> Account:
        data = await self._t.request("GET", "/accounts/get", params={"id": account_id})
