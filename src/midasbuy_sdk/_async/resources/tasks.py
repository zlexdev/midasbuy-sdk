    ...
    async def batch(
        ...
        *,
        ...
        data = await self._t.request(
            "POST", "/tasks/batch", json=body, idempotency_key=idempotency_key
        )
    ...
    async def package(
        ...
        *,
        ...
        if webhook_url is not None:
            body["webhook_url"] = webhook_url
        data = await self._t.request(
            "POST", "/tasks/package", json=body, idempotency_key=idempotency_key
        )
        return TaskAccepted.model_validate(data)

    async def get(self, task_id: str) -> TaskResult:
        data = await self._t.request("GET", "/tasks/get", params={"id": task_id})
        return TaskResult.model_validate(data)

    async def list(self, *, limit: int | None = None, offset: int = 0) -> Page[TaskSummary]:
        lim = self._limit(limit)
        data = await self._t.request("GET", "/tasks/list", params={"limit": lim, "offset": offset})
